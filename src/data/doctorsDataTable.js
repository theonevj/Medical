...
export const columns = (handleOpenUpdateData, handleOpenConfirmPopUp, fetchData) => [
  {
    field: 'drName',
      ...
      <div className="flex gap-3 items-center w-full h-full">
        {params.row?.approvalStatus === 'Approved' ? (
          ...
          : params.row?.approvalStatus === 'Rejected' ? (
            ...
            <>
              <button
                onClick={() => handleStatusChange(params.row, 'approve', () => { }, fetchData)}
                className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-md text-sm"
              >
                Accept
              </button>
              <button
                onClick={() => handleStatusChange(params.row, 'reject', () => { }, fetchData)}
...
const handleStatusChange = async (doctor, status, handleUpdateLocalStatus, fetchData) => {
  try {
    const response = await api.post(
      `/Doctor/${doctor?.drCode}/${status}`,
      {
        approvedBy: 0,
    ...
    if (response.status === 200) {
      ...
      await fetchData();
      handleUpdateLocalStatus(doctor.id, status);
    }
  } catch (error) {
