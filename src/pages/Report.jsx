         ...
         <div className='bg-white p-4 custom-shadow rounded-md flex flex-col gap-4'>
            ...
            <div className='grid md:grid-cols-2 grid-cols-1 gap-4 items-center'>
               ...
               <div className='flex flex-col gap-1'>
                  ...
                  <input value={fromDate} onChange={(e) => setFromDate(e.target.value)} type='date' max={new Date().toISOString().split("T")[0]} className='p-2 outline-none border rounded-md border-neutral-400'></input>
                  {errors.fromDate && <span className='text-sm text-red-500'>{errors.fromDate}</span>}
               </div>

               <div className='flex flex-col gap-1'>
                  <label>To Date</label>
                  <input value={toDate} onChange={(e) => setToDate(e.target.value)} type='date' max={new Date().toISOString().split("T")[0]} className='p-2 outline-none border rounded-md border-neutral-400'></input>
               </div>
