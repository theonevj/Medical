...
const Place = ({ place, index, endPoint, removePlace, movePlace }) => {
  ...
  return (
    <div
      ...
      className="p-3 hover:shadow cursor-move transition-all duration-300 px-4 flex justify-between border rounded-md bg-slate-50"
    >
...
function UpdateStourPlan() {
  ...
  const handleSelectAllowance = (item) => {
    const existItems = selectedAllowance.find(
      (i) => item.allowanceID === i.allowanceID
    );
    if (existItems) {
      setSelectedAllowanace(() =>
        selectedAllowance.filter((i) => i.allowanceID !== item.allowanceID)
      );
    } else {
      ...
      <div className="flex h-full flex-col gap-3 md:gap-4">
        ...
        <div className="bg-white h-full custom-shadow flex flex-col gap-4 rounded-md md:py-4 py-3 px-3 md:px-4">
          <div className="flex flex-col mb-1 gap-2">
            <label htmlFor="tourName" className="font-bold">
              Tour Plan Name <span className="text-red-500">*</span>
            </label>
            <input
          ...
          <div className="flex md:flex-row flex-col w-full items-start gap-4 md:gap-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="font-bold" htmlFor="perKm">
                Per Km <span className="text-red-500">*</span>
              </label>
              <input
                value={perKm}
                ...
                id="perKm"
                type="number"
                className="border outline-none py-.5 px-2 rounded-md"
                placeholder="Enter km"
              ></input>
              {errors.perKm && (
                <span className="text-sm text-red-500">{errors.perKm}</span>
              )}
            </div>

            <div className="flex w-52 flex-col gap-2">
              <label className="font-bold" htmlFor="allowance">
                ...
                {open && (
                  <div className="absolute h-24 overflow-scroll w-full shadow bg-white z-40">
                    {allowance.map((item, index) => (
                      <div
                        key={index}
                        className="grid p-2 grid-cols-4 items-start gap-2"
                      >
                        <input
                        ...
                        <span className="col-span-3 text-sm">
                          {item.allowanceName} (Rs. {item.allowanceAmount})
                        </span>
                      </div>
                    ))}
                  </div>
                )}
                {errors.selectedAllowance && (
            ...
            <div className="flex w-52 flex-col gap-2">
              <label>
                HeadQuaters <span className="text-sm text-red-500">*</span>
              </label>
              <select
          ...
          <div className="grid grid-cols-4 items-start w-3/5 gap-4">
            <div className="flex col-span-3 flex-col gap-1">
              <input
                ...
                value={newPlace}
                type="text"
                className="col-span-3 outline-none p-2.5 border rounded-md "
                placeholder="Enter place name..."
              ></input>
              {errors.places && (
                <span className="text-sm text-red-500">{errors.places}</span>
              )}
            </div>
            <button
              onClick={addNewPlace}
              className="p-2.5 md:w-36 w-36 col-span-1 flex justify-center items-center gap-2 text-white bg-themeblue rounded-md font-medium"
            >
              <span>
            ...
            {places.length > 0 ? (
              ...
              <div className="flex justify-center m-auto items-center">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <h1>
                    No places added yet. Start by adding your first destination!
                  </h1>
                </div>
              </div>
            )}
          ...
          {places.length > 0 && (
            ...
            >
              <button className="p-2 transition-colors duration-300 hover:bg-blue-600 bg-blue-500 w-36 rounded-md text-white">
                Submit
              </button>
            </div>
          )}
...
export default UpdateStourPlan;
